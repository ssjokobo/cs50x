...
bool check_circle(int win, int lose);
...
//helper function checking the circle
bool check_circle(int win, int lose)
{
    for (int i = 0; i < pair_count; i++)
    {
        if (locked[pairs[i].winner][win])
        {
            if (pairs[i].winner == lose)
            {
                return false;
            }
            else
            {
                if (check_circle(pairs[i].winner, lose))
                {
                    continue;
                }
                else
